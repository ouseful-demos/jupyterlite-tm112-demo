...
```{pyodide}
# Run this cell first to import the folium package
# to support map creation
import folium
```
...
#Save the mapmymap.save('OUcampusMap-annotated.html')
...
As well as the `folium` `ClickForMarker()` tool, there is another function we can add to the map - `LatLngPopup()`.Rather than placing a marker, this simply displays a popup revealing the latitude and longitude of the location clicked on.
...
# ...within which you can interactively display
# the latitude and longitude of clicked on locations
...
```{pyodide}
# Programmatically add two or three markers,
# with appropriate labels, 
# locating nearby points of interest
